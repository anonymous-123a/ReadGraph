...
class DynamicDataset_loader(dataset):
    c = 0.15
    k = 5
    eps = 0.001
    window_size = 1
    ...
    def __init__(self, args, seed=None, dName=None, dDescription=None):
        super(DynamicDataset_loader, self).__init__(dName, dDescription)
        self.index = args.data_index
    ...
    def get_adjs(self, rows, cols, weights, nb_nodes):
        if not os.path.exists('data/eigen/' + f'{self.dataset_name}/'):
            os.makedirs('data/eigen/' + f'{self.dataset_name}/')
        ...
        for i in range(len(rows)):
            ...
            adj = sp.csr_matrix((weights[i], (rows[i], cols[i])), shape=(nb_nodes, nb_nodes), dtype=np.float32)     #对每个切片进行稀疏邻接矩阵的生成
            adjs.append(self.preprocess_adj(adj))       #归一化并转化成稀疏张量
            if self.compute_s:
                if generate_eigen:
    ...
    def load(self):
        ...
        edges = [np.concatenate((np.vstack((rows[i], cols[i])).T, np.expand_dims(np.array(typs[i]), axis=1)), axis=1) for i in range(num_snap)]      #list[ndarray[n,3],]
        ...
        return {'X': None, 'A': adjs, 'S': eigen_adjs, 'index_id_map': index_id_map, 'edges': edges,
                ...
                'snap_test': snap_test, 'num_snap': num_snap, 'id_type_map':id_type_map}
